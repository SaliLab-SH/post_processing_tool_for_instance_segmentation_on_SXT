...
# from blob_algorithm_v1 import K_Means
from blob_algorithm_v4 import *
import random
...
def generation_mrc():
    ...
    instance_tiff_mito = skimage.morphology.label(tiff_mito)


    plt.imshow(instance_tiff_mito[189])
    plt.show()
    plt.close()
    tiff_maxindex_ =np.max(instance_tiff_mito)
    print(tiff_maxindex_)

    

    final_tiff = np.zeros_like(tiff_mito)
    ...
    for mito_index in range(19,20):    # 17  19  25 ?21
        print('current granule index', mito_index)
        mask_index = mito_index
        temp_tiff = np.zeros_like(tiff_isg)
        temp_tiff[np.where(instance_tiff_mito == mito_index)] = 1
        coords = np.where(instance_tiff_mito == mito_index)
        ...
        roi01 = ( slice(max(0,int(np.min(coords[0]))), min(instance_tiff_mito.shape[0], 1+int(np.max(coords[0])) )),slice(max(0,int(np.min(coords[1]))-10), min(instance_tiff_mito.shape[1], int(np.max(coords[1])) +10)), slice(max(0,int(np.min(coords[2]))-10), min(instance_tiff_mito.shape[2], int(np.max(coords[2]))+10 ))) 
    ...
    #     print('roi',roi01)
        ...
        temp_mito_partial_tiff = temp_tiff[roi01]
        temp_mito_partial_mrc = raw_mrc[roi01]
    
        shape_ = temp_mito_partial_tiff.shape

        # for i in range(int(shape_[0])):
        # # for i in range(1):
        #     plt.imshow(temp_mito_partial_tiff[i])  
        #     plt.show()
        #     plt.close()
        #     plt.imshow(temp_mito_partial_mrc[i])  
        #     plt.show()
        #     plt.close()

        # plt.imshow(temp_mito_partial_tiff[int(shape_[0]/2)])  
        # plt.show()
        # plt.close()
        # plt.imshow(temp_mito_partial_mrc[int(shape_[0]/2)])  
        # plt.show()
        # plt.close()

        test_mask = f'{test_datafile_name}/822_4_test_mito_mask.tiff'
        test_mrc = f'{test_datafile_name}/822_4_test_mito_mrc.tiff'

        if not os.path.exists(test_mask):  
            tifffile.imsave(test_mask, temp_mito_partial_tiff)
            tifffile.imsave(test_mrc, temp_mito_partial_mrc)
            print('saved.')
        else:
...
def blob_fit(mask,  mrc, min_r = 1.5, theta = 0.8, check_ = False):  
    ...
    # classify coords  
    mito_classification = blob_classification(mask, mrc)
    mito_classification.fit(final_cent_coord_lst, final_cent_rad_lst)
    labeled_index = mito_classification.labeled_center_index
    # labeled_index match with final_cent_coord_lst
    label_for_coord = [0]* len(final_cent_coord_lst)
    for i in range(len(labeled_index)):
        for coordindx in labeled_index[i]:
            label_for_coord[coordindx] = i + 1

    # print('line465', label_for_coord)
    ...
    def set_label(idx):
        ...
        index = argmin([ratio_d])
        return label_for_coord[index] + 1
...
def main():
    ...
    # mask = tifffile.imread(f'E:/PBC_data/datasets/tiff_24datasets_organelles/822_4_mito_label.tiff')
    ...
    print('index in total', index_seq[-1])
    ...
    tifffile.imsave('test_mito_instance_mask.tiff', instance_mask)
    ...
    plt.show()
    plt.close()
    print('label index', list(set(list(np.where(instance_mask.reshape(1, -1)))[0])))
